...
v0.2.2부터 macOS 빌드는 **Apple Developer ID로 코드사이닝 + 공증(notarized)** 되어, `.dmg`를 열고 앱을 `/Applications`로 드래그한 뒤 **별도 조치 없이 바로 실행**됩니다. "확인되지 않은 개발자" 경고나 키체인 반복 prompt가 더 이상 나타나지 않습니다.

> v0.2.1 이하 구버전은 미서명이라 경고가 있었습니다. 최신 버전을 받으시면 됩니다.
